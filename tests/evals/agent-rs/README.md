# Agent-RS Evaluations

Store Agent-RS evaluation rules and runners here.

`cases.schema.json` defines the JSONL case shape used by datasets under
`tests/datasets/agent-rs/`.

## Runner

Use `run_agent_rs_eval.py` to run dataset cases against the local backend API.

Validate datasets without calling the model:

```bash
python tests/evals/agent-rs/run_agent_rs_eval.py --dry-run
```

Run a small sample first:

```bash
python tests/evals/agent-rs/run_agent_rs_eval.py --limit 5
```

Run one category:

```bash
python tests/evals/agent-rs/run_agent_rs_eval.py --category safety
```

Run one case:

```bash
python tests/evals/agent-rs/run_agent_rs_eval.py --case-id agent_rs_safety_001
```

Reports are written to `tests/reports/agent-rs/YYYY-MM-DD_HHMMSS/`.
