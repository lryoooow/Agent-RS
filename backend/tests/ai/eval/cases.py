...
class AutogenEvalCase:
    ...
    # 历史对话干扰（random-stress 用）：注入到 AutoGen 回合上下文的前序轮次。
    ...
    def conversation_id(self) -> str:
        return f"autogen-eval-{self.case_id}"
...
GOLDEN_CASES: tuple[AutogenEvalCase, ...] = (
    AutogenEvalCase(
    ...
    AutogenEvalCase(
        case_id="tool_look_at_location",
        query="把地图定位到杭州西湖",
        expected_action="call",
        expected_capability="look_at_location",
        expected_arguments_subset={"query": "杭州西湖"},
        category="tool_positive",
...
def valid_capability_names() -> set[str]:
    return set(TOOLS) | {"web_search"}


def _bfcl_bucket(case: AutogenEvalCase) -> str:
...
def _is_burned_heldout_v1_case(case: AutogenEvalCase) -> bool:
...
GENERATED_CASES: tuple[AutogenEvalCase, ...] = generate_generated_cases() + HELDOUT_V1_BURNED_CASES
EVAL_CASES: tuple[AutogenEvalCase, ...] = GOLDEN_CASES + GENERATED_CASES
CI_REPLAY_CASES: tuple[AutogenEvalCase, ...] = GOLDEN_CASES
...
DEV_SET_CASES: tuple[AutogenEvalCase, ...] = EVAL_CASES
