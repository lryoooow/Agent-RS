...
本模块只提供图的构造；「什么时候走图、什么时候走 selector」是编排层的路由决策，
当前默认全部走 selector，图作为可选加速路径，由调用方显式选用。
"""
...
from app.agent.engine.agents import build_domain_agent, domain_specs
...
# （见 domain_agents.DOMAIN_GUIDANCE["preprocess_agent"]），
...
def build_flow(
    ...
    *,
    ...
    for domain in sequence:
        ...
        agents.append(
            build_domain_agent(specs[domain], model_client=model_client, memory=memory)
        )
    ...
    return GraphFlow(
        ...
        termination_condition=MaxMessageTermination(len(agents) * 6),
    )
