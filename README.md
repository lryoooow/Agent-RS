...
**准入与多用户**（内测准入，详见[访问控制](#访问控制邀请码--管理)）
- 邀请码注册 + 强制登录，按用户隔离数据；管理员可签发邀请码、管理用户
- 登录硬化：PBKDF2-SHA256（600k 迭代）、登录限流、防账号枚举

## 更新日志

### 2026-06-23 · 邀请码准入 + 登录硬化 + 管理体系

面向定向内测，补齐成熟 SaaS 的准入层：

- **邀请码注册**：开放注册改为邀请码准入（`INVITE_REQUIRED`）。邀请码高熵单次/限时，DB 仅存 HMAC，明文仅创建时展示一次。注册在事务内「建号 → 消费邀请」，失败整体回滚，不会吞码。
- **强制登录（服务端）**：DB 启用时所有数据接口需登录，绕过前端直接打接口同样 401。
- **登录硬化**：PBKDF2 迭代 210k → **600k**（OWASP 2025 基线），旧哈希登录时透明升级；按邮箱登录限流 + 指数退避锁定；时序均衡防账号枚举。
- **管理 API + 界面**：管理员（`ADMIN_EMAILS`）可签发/撤销邀请码、列出/停用用户；停用即时使其会话失效。配套 `scripts.admin_bootstrap` 解决「首个管理员」鸡生蛋。
...
- **鉴权**：PBKDF2-SHA256（600k）+ HMAC 会话令牌 + 邀请码准入
...
# 【公网部署必改】会话令牌/邀请码的 HMAC 密钥，改成长随机串（openssl rand -hex 32）
AUTH_SECRET_KEY=change-this-to-a-long-random-string
INVITE_REQUIRED=true             # 邀请码准入；开放注册设 false
ADMIN_EMAILS=you@example.com     # 管理员邮箱（逗号分隔）
...
python -m scripts.admin_bootstrap mint-invite   # 用输出的邀请码 + ADMIN_EMAILS 邮箱注册首个管理员
```
