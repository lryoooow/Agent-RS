...
Agent-RS 是一个面向遥感影像分析的 AI 助手应用。支持日常对话、知识问答、联网查询、影像上传预览，以及基于上传影像执行 NDVI、光谱指数、影像质检和波段组合等遥感处理。
...
- 遥感工具：通过统一 RS Tools MCP Docker Tool 计算并展示 NDVI、光谱指数和波段组合结果图层

## 环境准备

- Python 3.11+
- Node.js 18+
- Docker Desktop（用于 RS Tools 工具容器）
...
- 遥感处理统一走 `rs-tools-mcp:0.1.0`，不再保留 NDVI 本地兜底路径。
...
```bash
python -m compileall -q backend/app backend/sql docker/rs_tools
